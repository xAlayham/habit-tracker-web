...
- **No token refresh.** Access tokens expire after 30 minutes, and there's no refresh-token flow — you just get logged out and have to sign in again. Expiry is handled reactively: the app only discovers a token is dead when a request comes back `401`, at which point it clears the token and redirects to login. Checking the token's `exp` claim client-side would catch it proactively instead of after a failed request.
